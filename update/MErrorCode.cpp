...
//..............................................................................................................................
//根据错误代码获取错误描述
MString MErrorCode::GetErrString(unsigned long lErrorCode)
{
	register int					i;
	MString							strtemp;
	...
	if ( lErrorCode >= MERROR_USER )
	{
		for ( i=0;i<sizeof(s_stErrorCodeTable) / sizeof(tagMErrorRecord);i++ )
		{
			...
			{
				strtemp.Format("<%s>%s",s_stErrorCodeTable[i].strClassName,s_stErrorCodeTable[i].strDescribe);
				return(strtemp);
			}
		}

		strtemp.Format("<MErrorCode>不能匹配的错误号(%d)，请与软件提供商联系。",lErrorCode);
		return(strtemp);
	}
	//操作系统错误
	else if ( lErrorCode >= MERROR_SYSTEM )
	{
		lErrorCode -= MERROR_SYSTEM;
									...
									NULL    ) != 0 && lpmsgabuf != NULL )
			{
				...
				strtemp.Format("<Windows>%s",lptempptr);
				::LocalFree(lpmsgabuf);
			}
			else
			{
				strtemp.Format("<MErrorCode>无法获取Windows错误代码的描述信息(%d)。",lErrorCode);
			}
	...
	{
		strtemp.Format("<MErrorCode>成功返回，没有错误。");
		return(strtemp);
	}
}
//------------------------------------------------------------------------------------------------------------------------------
